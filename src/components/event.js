import '../css/event.css';
import { useEffect, useRef, useState} from 'react';
import { withTranslation,useTranslation } from 'react-i18next';
const Event = () => {
  const section3Ref = useRef();
  const cardRefs = useRef([]);
  const [cardTransforms, setCardTransforms] = useState([]);
  const { t } = useTranslation();

  useEffect(() => {
    const wrapper = section3Ref.current;
    const cards = cardRefs.current.map((cardRef) => cardRef.current);
    const start = wrapper.offsetTop - 100;
    const end = wrapper.offsetTop + wrapper.offsetHeight - window.innerHeight * 1.2;
    const step = (end - start) / (cards.length * 2);

    const updateTransforms = () => {
      if(window.innerWidth<=1024){
        return ;
      }
      const transforms = cards.map((card, i) => {
        const s = start + step * i;
        const e = s + step * (cards.length + 1);
        if (window.scrollY <= s) {
          return 'translateX(100vw)';
        } else if (window.scrollY > s && window.scrollY <= e) {
          return `translateX(${100 + (window.scrollY - s) / (e - s) * -100}vw)`;
        } else if (window.scrollY > e) {
          return 'translateX(0vw)';
        }
      });
      setCardTransforms(transforms);
    };

    updateTransforms();

    const handleResize = () => {
        updateTransforms();
    };

    const handleScroll = () => {
      updateTransforms();
    };

    window.addEventListener('resize', handleResize);
    window.addEventListener('scroll', handleScroll);

    return () => {
      window.removeEventListener('resize', handleResize);
      window.removeEventListener('scroll', handleScroll);
    };
  }, []);

    //href로 페이지 이동
    const MoveEventPage = ()=>{
      window.location.href = '/Event'
    }

  return (
    <section className="event w1500" ref={section3Ref} >
      <article className="sticky">
        <h1 className='title_h1 marginTop190pxW100per'>{t('sec3.title')}</h1>
        <div className="eventWrap">
          <div className='eventBox' onClick={MoveEventPage} key='0' style={{transform:cardTransforms[0]}} ref={(cardRef) => (cardRefs.current[0] = cardRef)}>
              <div className={`front front1`}></div>
              <div className="back">{t('sec3.box1')}</div>
          </div>
          <div className='eventBox' onClick={MoveEventPage} key='1' style={{transform:cardTransforms[1]}} ref={(cardRef) => (cardRefs.current[1] = cardRef)}>
              <div className={`front front2`}></div>
              <div className="back">{t('sec3.box2')}</div>
          </div>
          <div className='eventBox' onClick={MoveEventPage} key='2' style={{transform:cardTransforms[2]}} ref={(cardRef) => (cardRefs.current[2] = cardRef)}>
              <div className={`front front3`}></div>
              <div className="back">{t('sec3.box3')}</div>
          </div>
          <div className='eventBox' onClick={MoveEventPage} key='3' style={{transform:cardTransforms[3]}} ref={(cardRef) => (cardRefs.current[3] = cardRef)}>
              <div className={`front front4`}></div>
              <div className="back">{t('sec3.box4')}</div>
          </div>
        </div>
      </article>
    </section>
  );
};
export default withTranslation()(Event);